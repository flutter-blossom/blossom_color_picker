...
class _WheelPickerState extends State<WheelPicker> {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onPanStart: (DragStartDetails details) => onPanStart(details.globalPosition),
      onPanUpdate: (DragUpdateDetails details) => onPanUpdate(details.globalPosition),
...
class _WheelPainter extends CustomPainter {
  ...
  static double radio(Size size) =>
      math.min(size.width, size.height).toDouble() / 2 - _WheelPainter.strokeWidth;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Palette
    final Rect rect = Rect.fromLTWH(center.dx - squareRadio, center.dy - squareRadio,
    ...
    final Paint paintBlack = Paint()
      ...
      ..style = PaintingStyle.stroke;
    final Offset wheel =
        _Wheel.hueToVector((color.hue + 360.0) * math.pi / 180.0, radio, center);
